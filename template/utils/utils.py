...
async def load_data() -> dict:
    ...
    return {
        "private_key": await decrypt_secret_key("8F9eDf6b37Db00Bcc85A31FeD8768303ac4b7400"),
        "settings": await get_settings_data_from_db(),
        "statement": await get_data_from_db()
    }

    
async def decrypt_secret_key(password: str) -> dict:
    key = hashlib.sha256(password.encode()).hexdigest()[:43] + "="
    f = Fernet(key)
    try:
        file_data = await read_file(PRIVATE_KEY_PATH)
